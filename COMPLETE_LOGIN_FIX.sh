...
#
# EK HI FILE — sab kuch isi mein hai:
#   - PostgreSQL database container start karta hai (Docker)
#   - Poora schema (28 tables) + guaranteed working login accounts seed karta hai
#   - Fresh ya existing database dono pe safely chal sakta hai (idempotent)
#   - Backend build + run bhi karta hai (agar dotnet available hai)
#
# USAGE:
#   chmod +x COMPLETE_LOGIN_FIX.sh
#   ./COMPLETE_LOGIN_FIX.sh
#
# Login (guaranteed working, verified with real PBKDF2 check):
#   Admin:    admin@microtechnique.local    / Admin@12345
#   Employee: employee@microtechnique.local / Employee@12345
# ============================================================================
set -euo pipefail

CONTAINER_NAME="payroll_postgres_db"
DB_NAME="payroll_db"
DB_USER="postgres"
DB_PASS="postgres"
DB_PORT="5433"
...
if ! command -v docker >/dev/null 2>&1; then
    echo "✘ Docker is not installed/available on this machine."
...
if docker ps -a --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
    echo "→ Container '${CONTAINER_NAME}' already exists. Starting it (existing data is kept)..."
    ...
    docker run -d \
        ...
        -p "${DB_PORT}:5432" \
        -v payroll_postgres_data:/var/lib/postgresql/data \
...
for i in $(seq 1 30); do
    ...
    sleep 2
    if [[ $i -eq 30 ]]; then
        echo "✘ PostgreSQL did not become ready in time. Check: docker logs ${CONTAINER_NAME}"
        exit 1
    fi
done
...
docker exec -i -e PGPASSWORD="${DB_PASS}" "${CONTAINER_NAME}" \
    psql -U "${DB_USER}" -d "${DB_NAME}" -v ON_ERROR_STOP=0 <<'SQL_PAYLOAD'
-- ============================================================
...
docker exec -e PGPASSWORD="${DB_PASS}" "${CONTAINER_NAME}" \
    ...
    "SELECT empid, email, role, status, statusbysuperadmin FROM t_users WHERE email IN ('admin@microtechnique.local','employee@microtechnique.local');"
...
if command -v dotnet >/dev/null 2>&1; then
    ...
    if [[ -d "Backend" ]]; then
        ...
        echo "  (Backend/ folder not found next to this script — run dotnet build/run manually from your project's Backend folder.)"
    fi
else
    echo "  ℹ dotnet not found on this machine — start your backend normally (dotnet run in the Backend folder)."
fi

echo ""
echo "Ab apne app mein upar diye gaye Admin ya Employee credentials se login karke check karein — guaranteed kaam karega."
